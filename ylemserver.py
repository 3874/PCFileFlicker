...
@QRapp.route('/search', methods=['POST'])
def search():
    data = request.get_json()
    
    # data가 리스트인지 확인
    if isinstance(data, list) and len(data) > 0:
        data = data[0]  # 첫 번째 요소를 사용
    
    query = data.get('query')
    if not query:
        return jsonify({'error': '검색어가 필요합니다.'}), 400

    keyword_prompt = "다음 문장에서 키워드를 추출해서 스페이스로 구분하는 문자열로 만들어줘: "
    search_prompt = "다음 질문에 답을 하는데 적절한 답이 없다면 'no_data'라고 답해줘: "
    final_prompt = "다음 내용을 정리해서 html 형태로만 답을 줘: "

    try:
        result = ai_analysis(search_prompt, query)  # 순차적으로 result 값을 받아옴
        if 'no_data' in result:  # result에 no_data가 포함되어 있는지 확인
            keyword = ai_analysis(keyword_prompt, query)  # keyword 추출
            result2 = search_google(keyword, 10)  # search_google 호출
            result3 = ai_analysis(final_prompt, result2)
            return jsonify({'query': query, 'result': result3}), 200  # 결과 반환
        return jsonify({'query': query, 'result': result}), 200  # result 반환
...
def search_google(query, num_results=10):
    ...
    :param query: 검색할 쿼리
    :param num_results: 받으려는 결과 수 (기본값: 10)
    :return: 검색 결과 JSON
    """
